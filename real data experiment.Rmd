...
```{r}
structure=TRUE
model1=ranger(Surv(time, status) ~ .,data=train,num.tree=500,mtry=ncol(train)-2,splitrule = "logrank", min.node.size = 25)

set.seed(109) # seed 109, num_leaf=4, min leaf size=13
# seed 7305, min leaf size=11

model2=ranger(Surv(time, status) ~ .,data=train,num.tree=1,mtry=ncol(train)-2,splitrule = "logrank", min.node.size = 25) # sample without rep
...
#change the group from 1 3 4 5 to 1 2 3 4
...
```
* Access the tree strucutre info.
```{r}
treeInfo(model2)
reassign_dict
```

Now fit the model on the full data
```{r}
# Use the full group
...
* Repeat the above process for another dataset
```{r}
library(dplyr)
data3 = read_csv("/Users/fanyiyang/Desktop/Research/Survival\ Tree/real\ data/S1Data.csv")
data3=rename(data3,"time"="TIME","status"="Event")
sum(data3$status==1)/nrow(data3)
hist(data3$time,breaks=seq(0,300,10))
```
```{r}
train=read_csv("/Users/fanyiyang/Desktop/Research/Survival\ Tree/real\ data/train1.csv")
```
...
```{r}
set.seed(347)
index = sample(nrow(data3), 240, replace = F)
train = data3[index,]
test = data3[-index,]
sum(train$status==1)/nrow(train)
tau=max(data3$time)
```

```{r}
structure=TRUE
model1=ranger(Surv(time, status) ~ .,data=train,num.tree=500,mtry=ncol(train)-2,splitrule = "logrank", min.node.size = 50)

set.seed(521) # seed 109, num_leaf=4, min leaf size=13
# seed 7305, min leaf size=11

model2=ranger(Surv(time, status) ~ .,data=train,num.tree=1,mtry=ncol(train)-2,splitrule = "logrank", min.node.size = 50) # sample without rep
# model2=rfsrc(Surv(time, status) ~ ., data=train, mtry=ncol(train)-2,sampsize=nrow(train),nsplit=0,ntree=1) # sample without rep
...
predict2=predict(model2, data=data3)
chf_time=predict2$unique.death.times
chf_value=predict2$chf
...
par(mfrow = c(1, 1))  # Set the number of rows and columns for plots
par(plt = c(0.3, 0.58, 0.3, 0.9))  # Set margins for the plot
plot(seq(tau/num_interval,tau,tau/num_interval),result,type="l",xlab="Time", ylab="Survival Probability",ylim=c(0,1),main="Node 10")
```
