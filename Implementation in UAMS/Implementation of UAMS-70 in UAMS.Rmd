---
title: "Implementation of UAMS70"
...
Get the probe set IDs and weights in the UAMS70 model
```{r}
...
KM=autoplot(PFS_fit,main="PFS of different risk groups in UAMS categorized by UAMS-70")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/UAMS-UAMS70-PFS.pdf",
       device = "pdf")
```
```{r}
...
KM=autoplot(OS_fit,main="OS of different risk groups in UAMS categorized by UAMS-70",xlim=c(0,3000))+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/UAMS-UAMS70.pdf",
       device = "pdf")
write(high_risk, "./Categorization/UAMS-UAMS70.txt")
```
```{r}
...
```{r}
UAMS70_score=data.frame(score)
UAMS70_score$patient=patient
write.csv(UAMS70_score,"UAMS70_score_UAMS.csv",row.names=F)
```
