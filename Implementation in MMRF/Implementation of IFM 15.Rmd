...
TPM_first_IFM = subset(TPM_first_visit,Gene %in% dict$ensembl_gene_id)
tidydata=t(TPM_first_IFM)%>%
...
```{r}
library(readr)
HGU133Plus2_Hs_ENSG_mapping <- read_delim("/Users/fanyiyang/Desktop/Research/summer/Implementation\ of\ risk\ models/HGU133Plus2_Hs_ENSG_mapping.txt", 
...
# 15 genes from brainarray, 1 genes from bioMart
```
...
```{r}
MMRF_CoMMpass_IA21_STAND_ALONE_SURVIVAL=read_tsv("/Users/fanyiyang/Desktop/Research/summer/Implementation\ of\ risk\ models/MMRF_CoMMpass_IA21_STAND_ALONE_SURVIVAL.tsv")
...
autoplot(OS_fit,main="OS of different risk groups categorized by IFM15")
```
```{r}
...
```{r}
IFM_score=data.frame(score)
IFM_score$patient=gsub(pattern = "_1_BM_CD138pos",replacement = "",x = patient)
write.csv(IFM_score,"IFM_score.csv",row.names=F)
```
