library(readr)
setwd("/Users/fanyiyang/Desktop/Research/summer/Ensemble-tree-for-risk-prediction-in-MM/Implementation\ in\ MMRF")
MMRF <- read.csv("./merged_MMRF_scores.csv")
binary <- MMRF
...
GPI_threshold = quantile(MMRF[,4],0.92)
binary[,4] = ifelse(MMRF[,4]>GPI_threshold,1,0)
...
EMC_model <- coxph(Surv(ttcpfs, censpfs) ~ EMC,data = binary)
UAMS70_model <- coxph(Surv(ttcpfs, censpfs) ~ UAMS70,data = binary)
UAMS80_model <- coxph(Surv(ttcpfs, censpfs) ~ UAMS80,data = binary)
UAMS17_model <- coxph(Surv(ttcpfs, censpfs) ~ UAMS17,data = binary)
IFM15_model <- coxph(Surv(ttcpfs, censpfs) ~ IFM15,data = binary)
HM19_model <- coxph(Surv(ttcpfs, censpfs) ~ HM19,data = binary)
GPI_model <- coxph(Surv(ttcpfs, censpfs) ~ GPI,data = binary)
...
ggplot(combined_df, aes(x = Variable, y = HR, ymin = Lower, ymax = Upper, color = Model)) +
  ...
  labs(title = "Forest Plot of individual signature models in MMRF PFS",
       y = "Hazard Ratio (HR)",
       x = "")
...
ggplot(combined_df, aes(x = Variable, y = HR, ymin = Lower, ymax = Upper, color = Model)) +
  ...
  labs(title = "Forest Plot of individual signature models in MMRF OS",
       y = "Hazard Ratio (HR)",
       x = "")