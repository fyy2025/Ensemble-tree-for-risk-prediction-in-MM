...
KM=autoplot(PFS_fit,main="PFS of different risk groups in MMRF categorized by EMC-92")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/MMRF-EMC92-PFS.pdf",
       device = "pdf")
# write(high_risk, "./Categorization/EMC92-PFS.txt")
```
...
KM=autoplot(OS_fit,main="OS of different risk groups in MMRF categorized by EMC-92")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/MMRF-EMC92.pdf",
       device = "pdf")
write(high_risk, "./Categorization/EMC92.txt")
```
```{r}
...
EMC_score$patient=gsub(pattern = "_1_BM_CD138pos",replacement = "",x = patient)
write.csv(EMC_score,"EMC_score.csv",row.names=F)
```
