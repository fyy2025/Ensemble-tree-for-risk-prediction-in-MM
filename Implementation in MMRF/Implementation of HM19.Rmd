...
Get the probe set IDs and weights in the HM19 model
```{r}
library(geneClassifiers)
HM19Classifier<-getClassifier("HM19")
HM19Classifier
weights=getWeights(HM19Classifier)
weights
...
# 16 genes from brainarray, 2 genes from bioMart
```
...
autoplot(PFS_fit,main="PFS of different risk groups categorized by HM19")
```
```{r}
...
autoplot(OS_fit,main="OS of different risk groups categorized by HM19")
```
```{r}
...
```{r}
HM_score=data.frame(score)
HM_score$patient=gsub(pattern = "_1_BM_CD138pos",replacement = "",x = patient)
write.csv(HM_score,"HM_score.csv",row.names=F)
```
