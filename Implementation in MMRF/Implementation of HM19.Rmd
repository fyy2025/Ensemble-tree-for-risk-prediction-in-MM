...
KM=autoplot(PFS_fit,main="PFS of different risk groups in MMRF categorized by HM-19")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/MMRF-HM19-PFS.pdf",
       device = "pdf")
```
```{r}
...
KM=autoplot(OS_fit,main="OS of different risk groups in MMRF categorized by HM-19")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/MMRF-HM19.pdf",
       device = "pdf")
write(high_risk, "./Categorization/HM19.txt")
```
```{r}
