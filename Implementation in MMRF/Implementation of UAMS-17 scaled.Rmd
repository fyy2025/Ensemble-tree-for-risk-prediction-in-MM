...
write.csv(UAMS_score,"UAMS17_score_MMRF.csv",row.names=F)
```
...
KM=autoplot(PFS_fit,main="PFS of different risk groups in MMRF categorized by UAMS17",ylim = c(0, 1.0))+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/MMRF-UAMS17-PFS.pdf",
       device = "pdf")
```
...
KM=autoplot(OS_fit,main="OS of different risk groups in MMRF categorized by UAMS17",ylim = c(0, 1.0))+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/MMRF-UAMS17.pdf",
       device = "pdf")
write(high_risk, "./Categorization/UAMS17.txt")
```
```{r}
