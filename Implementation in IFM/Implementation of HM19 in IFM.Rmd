...
KM=autoplot(PFS_fit,main="PFS of different risk groups in IFM categorized by HM-19")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/IFM-HM19-PFS.pdf",
       device = "pdf")
```
```{r}
...
KM=autoplot(OS_fit,main="OS of different risk groups in IFM categorized by HM-19")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/IFM-HM19.pdf",
       device = "pdf")
write(high_risk, "./Categorization/IFM-HM19.txt")
```
```{r}
...
write.csv(HM19_score,"HM19_score_IFM.csv",row.names=F)
```
