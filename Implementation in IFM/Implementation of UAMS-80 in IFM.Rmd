...
KM=autoplot(PFS_fit,main="PFS of different risk groups in IFM categorized by UAMS-80")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/IFM-UAMS80-PFS.pdf",
       device = "pdf")
```
```{r}
...
KM=autoplot(OS_fit,main="OS of different risk groups in IFM categorized by UAMS-80")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/IFM-UAMS80.pdf",
       device = "pdf")
write(high_risk, "./Categorization/IFM-UAMS80.txt")
```
```{r}
