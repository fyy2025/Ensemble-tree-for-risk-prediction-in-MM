...
```{r}
set.seed(1)
index = sample(nrow(merged), 400, replace = F)
...
# library(rpart)
# library(party)
# index = sample(nrow(merged_data01), 400, replace = F)
# train = merged_data01[index,]
# model_prune=rpart(Surv(time, status) ~ .,data=train)
# model_conditioned=ctree(Surv(time, status) ~ .,data=train)
```
