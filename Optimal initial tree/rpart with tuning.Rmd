...
hist(merged_data$time)
tau=max(merged_data$time)
```

```{r}
prune_initial_loss=rep(0,500)
prune_super_loss=rep(0,500)
forest_loss=rep(0,500)
num_leaves=rep(0,500)
prune_initial_loss01=rep(0,500)
prune_super_loss01=rep(0,500)
forest_loss01=rep(0,500)
num_leaves01=rep(0,500)
...
for (seed in 1:500){
  ...
  fit=rpart(Surv(time, status) ~ .,data=train,minbucket=nrow(train)%/%10,cp=0)
  cp_table <- fit$cptable
  # Identify the smallest xerror and its standard deviation
  min_xerror <- min(cp_table[, "xerror"])
  min_xerror_std <- cp_table[which.min(cp_table[, "xerror"]), "xstd"]
  
  # Apply the 1-SE rule
  one_se_rule <- min_xerror + min_xerror_std
  best_cp_index <- which(cp_table[, "xerror"] <= one_se_rule)[1]
  best_cp <- cp_table[best_cp_index, "CP"]
  model_prune <- prune(fit, cp = best_cp)
  ...
  prune_super_loss[seed]=cIndex(test$time,test$status,as.vector(result))[1]
  prune_initial_loss[seed]=test_c_index[1]
  ...
  prune_super_loss01[seed]=cIndex(test$time,test$status,as.vector(result))[1]
  prune_initial_loss01[seed]=test_c_index[1]
...
```{r}
summary2=cbind(forest_loss,forest_loss01,prune_initial_loss,prune_initial_loss01,prune_super_loss,prune_super_loss01,num_leaves,num_leaves01)
par(cex.axis=0.8, mar=c(8, 10, 5, 1))
boxplot(summary2[,1:6],beside=T,horizontal=T,las=1,xlab="C index")
```
```{r}
boxplot(cbind(num_leaves,num_leaves01))
```


```{r}
summary2=as.data.frame(summary2)
write.csv(summary2,"./summary.csv")
```
...
summary$prune_initial_ratio=summary2$prune_initial_loss/summary$initial_loss
summary$prune_super_ratio=summary2$prune_super_loss/summary$initial_loss
summary$prune_initial01_ratio=summary2$prune_initial_loss01/summary$initial_loss
summary$prune_super01_ratio=summary2$prune_super_loss01/summary$initial_loss

```

```{r}
par(cex.axis=0.8, mar=c(8, 10, 5, 1))
boxplot(summary[7:15],beside=T,horizontal=T,las=1,xlab="C index")
```

```{r}
prune_summary=cbind(prune_initial_loss,prune_initial_loss01,prune_super_loss,prune_super_loss01,num_leaves01,num_leaves)
```

```{r}
write.csv(prune_summary,"prune_summary.csv",row.names=F)
```



```{r}
summary=cbind(num_leaves01,num_leaves)
par(cex.axis=0.8, mar=c(8, 10, 5, 1))
