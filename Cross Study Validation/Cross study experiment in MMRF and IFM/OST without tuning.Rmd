...
```{r}
library(ranger)
ost_initial_loss=rep(0,500)
ost_super_loss=rep(0,500)
...
tau=max(merged_MMRF_data_continuous$time)
...
for (seed in 1:1){
  ...
  ost_super_loss[seed]=cIndex(test$time,test$status,as.vector(result))[1]
  ost_initial_loss[seed]=test_c_index[1]
...
test = drop_na(merged_IFM_data_binary)
tau=max(merge_MMRF_data_binary$time)
