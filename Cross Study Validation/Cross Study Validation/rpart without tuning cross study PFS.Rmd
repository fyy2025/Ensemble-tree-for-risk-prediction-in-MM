...
IFM_pfs=IFM_data_continuous[,c(1:7,10:11)]%>%
  rename(time=TTRelpase,status=Relapse)
IFM_pfs=IFM_pfs[IFM_pfs$time>0,]
MMRF_data_continuous=read_csv("MMRF_8_continuous.csv")[,-2]
MMRF_pfs=MMRF_data_continuous[,c(1:7,10:11)]%>%
  rename(time=ttcpfs,status=censpfs)
Gamer_data_continuous=read_csv("Gamer_7_continuous.csv")
Gamer_pfs=Gamer_data_continuous[,c(1:9)]%>%
  rename(time=relapse1,status=relapseE)
Gamer_pfs=Gamer_pfs[Gamer_pfs$time>0,]
UAMS_data_continuous=read_csv("UAMS_8_continuous.csv")[,-2]
UAMS_pfs=UAMS_data_continuous[,c(1:7,10:11)]%>%
  rename(time=EFS,status=EFS_censor)
UAMS_pfs=UAMS_pfs[UAMS_pfs$time>0,]
```
```{r}
IFM_pfs_binary=IFM_pfs
IFM_pfs_binary$EMC=ifelse(IFM_pfs$EMC>quantile(IFM_pfs$EMC,0.8),1,0)
IFM_pfs_binary$UAMS70=ifelse(IFM_pfs$UAMS70>quantile(IFM_pfs$UAMS70,0.8),1,0)
IFM_pfs_binary$GPI=ifelse(IFM_pfs$GPI>quantile(IFM_pfs$GPI,0.9),1,0)
IFM_pfs_binary$UAMS17=ifelse(IFM_pfs$UAMS17>quantile(IFM_pfs$UAMS17,0.9),1,0)
IFM_pfs_binary$UAMS80=ifelse(IFM_pfs$UAMS80>quantile(IFM_pfs$UAMS80,0.9),1,0)
IFM_pfs_binary$HM19=ifelse(IFM_pfs$HM19>quantile(IFM_pfs$HM19,0.8),1,0)
IFM_pfs_binary$IFM15=ifelse(IFM_pfs$IFM15>quantile(IFM_pfs$IFM15,0.8),1,0)
```

```{r}
MMRF_pfs_binary=MMRF_pfs
MMRF_pfs_binary$EMC=ifelse(MMRF_pfs$EMC>quantile(MMRF_pfs$EMC,0.8),1,0)
MMRF_pfs_binary$UAMS70=ifelse(MMRF_pfs$UAMS70>quantile(MMRF_pfs$UAMS70,0.8),1,0)
MMRF_pfs_binary$GPI=ifelse(MMRF_pfs$GPI>quantile(MMRF_pfs$GPI,0.9),1,0)
MMRF_pfs_binary$UAMS17=ifelse(MMRF_pfs$UAMS17>quantile(MMRF_pfs$UAMS17,0.9),1,0)
MMRF_pfs_binary$UAMS80=ifelse(MMRF_pfs$UAMS80>quantile(MMRF_pfs$UAMS80,0.9),1,0)
MMRF_pfs_binary$HM19=ifelse(MMRF_pfs$HM19>quantile(MMRF_pfs$HM19,0.8),1,0)
MMRF_pfs_binary$IFM15=ifelse(MMRF_pfs$IFM15>quantile(MMRF_pfs$IFM15,0.8),1,0)
```

```{r}
Gamer_pfs_binary=Gamer_pfs
Gamer_pfs_binary$EMC=ifelse(Gamer_pfs$EMC>quantile(Gamer_pfs$EMC,0.8),1,0)
Gamer_pfs_binary$UAMS70=ifelse(Gamer_pfs$UAMS70>quantile(Gamer_pfs$UAMS70,0.8),1,0)
Gamer_pfs_binary$GPI=ifelse(Gamer_pfs$GPI>quantile(Gamer_pfs$GPI,0.9),1,0)
Gamer_pfs_binary$UAMS17=ifelse(Gamer_pfs$UAMS17>quantile(Gamer_pfs$UAMS17,0.9),1,0)
Gamer_pfs_binary$UAMS80=ifelse(Gamer_pfs$UAMS80>quantile(Gamer_pfs$UAMS80,0.9),1,0)
Gamer_pfs_binary$HM19=ifelse(Gamer_pfs$HM19>quantile(Gamer_pfs$HM19,0.8),1,0)
Gamer_pfs_binary$IFM15=ifelse(Gamer_pfs$IFM15>quantile(Gamer_pfs$IFM15,0.8),1,0)
```

```{r}
UAMS_pfs_binary=UAMS_pfs
UAMS_pfs_binary$EMC=ifelse(UAMS_pfs$EMC>quantile(UAMS_pfs$EMC,0.8),1,0)
UAMS_pfs_binary$UAMS70=ifelse(UAMS_pfs$UAMS70>quantile(UAMS_pfs$UAMS70,0.8),1,0)
UAMS_pfs_binary$GPI=ifelse(UAMS_pfs$GPI>quantile(UAMS_pfs$GPI,0.9),1,0)
UAMS_pfs_binary$UAMS17=ifelse(UAMS_pfs$UAMS17>quantile(UAMS_pfs$UAMS17,0.9),1,0)
UAMS_pfs_binary$UAMS80=ifelse(UAMS_pfs$UAMS80>quantile(UAMS_pfs$UAMS80,0.9),1,0)
UAMS_pfs_binary$HM19=ifelse(UAMS_pfs$HM19>quantile(UAMS_pfs$HM19,0.8),1,0)
UAMS_pfs_binary$IFM15=ifelse(UAMS_pfs$IFM15>quantile(UAMS_pfs$IFM15,0.8),1,0)
```
...
library(caret)
train=drop_na(IFM)
folds <- createFolds(train$time, k = 5, list = TRUE, returnTrain = TRUE)
...
for (i in 1:5) {
  ...
  test_indices <- setdiff(seq_len(nrow(train)), train_indices)
  ...
  cindex <- cIndex(test_data$time,test_data$status,as.vector(risk_scores))[1]
...
```{r}
(cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$EMC)[1]+
cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$UAMS70)[1]+
cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$GPI)[1]+
cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$UAMS17)[1]+
cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$UAMS80)[1]+
cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$HM19)[1]+
cIndex(MMRF_pfs_binary$time,MMRF_pfs_binary$status,MMRF_pfs_binary$IFM15)[1])/7

```
```{r}
IFM=drop_na(IFM)
(
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$EMC)[1]+
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$UAMS70)[1]+
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$GPI)[1]+
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$UAMS17)[1]+
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$UAMS80)[1]+
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$HM19)[1]+
  cIndex(IFM_pfs_binary$time,IFM_pfs_binary$status,IFM_pfs_binary$IFM15)[1]
)/7
```
```{r}
(cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$EMC)[1]+
cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$UAMS70)[1]+
cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$GPI)[1]+
cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$UAMS17)[1]+
cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$UAMS80)[1]+
cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$HM19)[1]+
cIndex(UAMS_pfs_binary$time,UAMS_pfs_binary$status,UAMS_pfs_binary$IFM15)[1])/7
```
```{r}
(cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$EMC)[1]+
cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$UAMS70)[1]+
cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$GPI)[1]+
cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$UAMS17)[1]+
cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$UAMS80)[1]+
cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$HM19)[1]+
cIndex(Gamer_pfs_binary$time,Gamer_pfs_binary$status,Gamer_pfs_binary$IFM15)[1])/7
```
```{r}
hist(subset(UAMS,time<6000)$time)
```
...
datasets=list(MMRF_pfs_binary,IFM_pfs_binary,UAMS_pfs_binary,Gamer_pfs_binary)
...
for (train_index in 1:4){
  for (test_index in 1:4){
    ...
    # print(paste("ensemble: ",cIndex(test$time,test$status,as.vector(result))[1]))
    ensemble[train_index,test_index]=cIndex(test$time,test$status,as.vector(result))[1]
    # print(paste("initial: ",test_c_index[1]))
    initial[train_index,test_index]=test_c_index[1]
    ...
    # print(paste("forest: ",cIndex(test$time,test$status,as.vector(test_risk_score2))[1]))
    forest[train_index,test_index]=cIndex(test$time,test$status,as.vector(test_risk_score2))[1]
    ...
    # print(paste("coxph: ",cIndex(test$time,test$status,as.vector(risk_score))[1]))
    coxph[train_index,test_index]=cIndex(test$time,test$status,as.vector(risk_score))[1]
