...
times = seq(tau/k-1, tau-1, by = tau/k)
surv_fit <- survfit(baseline_model, newdata = test)
summary_fit=summary(surv_fit, times = times)
...
pseudo_observation=1-exp(pseudo(survfit(Surv(time, status) ~ 1, data=test),times=summary_fit$time,type="cumhaz"))
colnames(pseudo_observation)=c(summary_fit$time)

# glm=glm(pseudo_observation~revised_test_harzard)
# hypothesis_matrix <- c("(Intercept) = 0", "revised_test_harzard = 1")
# wald_test <- car::linearHypothesis(glm, hypothesis_matrix)

test_new=cbind(test,pseudo_observation)
long_data <- test_new %>%
  pivot_longer(
    cols = colnames(.)[grepl("^[[:digit:]]+", colnames(.))],
    names_to = "time_point",
    values_to = "pseudo_value"
  ) %>%
  mutate(time_point = as.numeric(gsub("time.", "", time_point)))
long_data$risk_score=c(predicted_probs)
long_data$log_risk_score=log(long_data$risk_score)

fit <- glm(pseudo_value ~ factor(time_point) + offset(log_risk_score) , data = long_data)
hypothesis_matrix <- c(names(fit$coefficients))
joint_test <- car::linearHypothesis(fit, hypothesis_matrix)
calibration_at_large_pvalue=joint_test$`Pr(>Chisq)`[2]
```
