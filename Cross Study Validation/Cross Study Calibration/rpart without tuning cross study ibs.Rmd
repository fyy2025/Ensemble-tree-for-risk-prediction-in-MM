...
for (train_index in 1:4){
  for (test_index in 1:4){
    ...
    # tau=landmark_tau
    tau=max(test$time)
    # timeOfInterest = unname(quantile(c(test$time),c(.1,.25,.5,.75,.9)))
    timeOfInterest=seq(tau/10,tau,tau/10)
    ...
    for (i in 1:num_leaf){
      ...
      for (u in 1:length(timeOfInterest)){
    ...
    for (i in 1:num_leaf){
      test_ibs[i]=integrated_brier_score(y_true=with(test,Surv(time,status)),surv=1-matrix(unlist(test_risk_score[[i]]), ncol = length(timeOfInterest), byrow = FALSE),times=timeOfInterest)
    }
    
    result=0
    for (i in 1:num_leaf){
      result=result+parameter[i]*matrix(unlist(test_risk_score[[i]]), ncol = length(timeOfInterest), byrow = FALSE) # calculating the ensemble risk score
    }
    
    # print(paste("ensemble: ",IBS(with(test,Surv(time,status)),sp_matrix =result)))
    # print(paste("initial: ",test_ibs[1]))
    ensemble[train_index,test_index]=integrated_brier_score(y_true=with(test,Surv(time,status)),surv=1-result,times=timeOfInterest)
    initial[train_index,test_index]=test_ibs[1]
    
    test_predict=predict(model1, data=test)
    chf_predict=test_predict$chf
    chf_predict_time=test_predict$unique.death.times
    test_risk_score2=matrix(0,nrow=nrow(test),ncol=length(timeOfInterest))
    ...
    forest[train_index,test_index]=integrated_brier_score(y_true=with(test,Surv(time,status)),surv=1-test_risk_score2,times=timeOfInterest)
