...
type="DN"
numBuckets=10
trueDeathTimes = test$time
censorStatus = test$status
timeOfInterest = unname(quantile(c(trueDeathTimes,train$time),c(.1,.25,.5,.75,.9)))
...
for(times in 1:length(timeOfInterest)){
  ...
  predictionsSpecified = predicted_probs[times,]
...
for (j in 1:nrow(test)){
  for (u in 1:length(timeOfInterest)){
    ...
    test_risk_score2[j,u]=exp(-chf_predict[j,index])
  }
}

predicted_probs=t(test_risk_score2)
...
for (train_index in 1:4){
  for (test_index in 1:4){
    ...
    baseline_model=coxph(Surv(time, status) ~., data = train, x=TRUE, y=TRUE)
    surv_fit <- survfit(baseline_model, newdata = test, extend=TRUE)
    deathProbabilities=rep(0,nrow(test))
    censorStatus=test$status
    quantiles = 1 - seq(0,1,length.out = numBins+1)
    for (i in 1:nrow(test)){
      deathProbabilities[i] <- summary(surv_fit, times = test$time[i], extend=TRUE)$surv[i]
    }
    ...
    coxph[train_index,test_index]=chisq.test(combinedBins)$p.value
    
    
    # death prob for initial tree
    ...
    for (j in 1:nrow(test)){
      ...
      for (i in 1:num_leaf){
        ...
        test_risk_score[[i]][j]=exp(-value[index])
        
      }
      
    }
    
    deathProbabilities=test_risk_score[[1]]
    uncensoredProbabilities = deathProbabilities[as.logical(censorStatus)]
    binIndex = seq_along(quantiles)[-1]
    uncensoredBinning = unlist(lapply(binIndex, function(x) length(which(uncensoredProbabilities >= quantiles[x] &
                                                                           uncensoredProbabilities < quantiles[x-1]))))
    ...
    combinedBins = uncensoredBinning + censoredBinning
    names(combinedBins) = quantiles[-length(quantiles)]
    initial[train_index,test_index]=chisq.test(combinedBins)$p.value
    ...
    combinedBins = uncensoredBinning + censoredBinning
    names(combinedBins) = quantiles[-length(quantiles)]
    ensemble[train_index,test_index]=chisq.test(combinedBins)$p.value
    ...
    for (j in 1:nrow(test)){
      ...
      test_risk_score2[j]=exp(-chf_predict[j,index])
    }
    
    deathProbabilities=test_risk_score2
    uncensoredProbabilities = deathProbabilities[as.logical(censorStatus)]
    binIndex = seq_along(quantiles)[-1]
    uncensoredBinning = unlist(lapply(binIndex, function(x) length(which(uncensoredProbabilities >= quantiles[x] &
                                                                           uncensoredProbabilities < quantiles[x-1]))))
    ...
    combinedBins = uncensoredBinning + censoredBinning
    names(combinedBins) = quantiles[-length(quantiles)]
    forest[train_index,test_index]=chisq.test(combinedBins)$p.value
