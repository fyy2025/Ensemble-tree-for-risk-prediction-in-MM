...
pseudo <- pseudosurv(bmt$tdfs, bmt$dfs, tmax=cutoffs)$pseudo
a=cbind(bmt,pseudo)
a[order(a$t1),]
b <- NULL
for(j in 1:ncol(pseudo)){
  b <- rbind(b,cbind(bmt,pseudo=pseudo[,j], tpseudo=cutoffs[j],id=1:nrow(bmt)))
}
b <- b[order(b$id),]
library(geepack)
b$tpseudo <- as.factor(b$tpseudo)
b$ipseudo <- 1-b$pseudo
summary(fit <- geese(ipseudo ~ tpseudo + disease + fab + age, data = b,        id=id, scale.fix=TRUE, family=gaussian, mean.link = "cloglog",corstr="independence"))
```
...
summary_fit=summary(surv_fit, times = times, extend=TRUE)
predicted_probs <- summary_fit$surv

fit=survfit(Surv(time,status)~1,data=test)
# pseudo_observation=1-pseudosurv(time=test$time,event=test$status,tmax=summary_fit$time)$pseudo
pseudo_cumhaz=pseudo(survfit(Surv(time, status) ~ 1, data=test),times=tau,type="cumhaz")

test_new=cbind(test,pseudo_cumhaz)
test_new$id=1:nrow(test_new)

long_data=test_new 
long_data$risk_score=c(predicted_probs)
long_data$log_hazard=log(-log(1-long_data$risk_score))
long_data=subset(long_data,abs(pseudo_cumhaz)<5)
long_data$pseudo_risk=1-exp(-long_data$pseudo_cumhaz)

geese_fit <- geese(as.formula(pseudo_risk ~ log_hazard) , id=id, data = long_data, scale.fix=TRUE, family=gaussian, mean.link = "cloglog", corstr="independence")

coef_estimates <- geese_fit$beta
cov_matrix <- geese_fit$vbeta

intercept_terms <- names(coef_estimates)
hyp_matrix <- diag(length(intercept_terms))
rownames(hyp_matrix) <- names(coef_estimates)[intercept_terms]
# 
# wald_test <- wald.test(Sigma = cov_matrix, b = coef_estimates, L = hyp_matrix)
L <- matrix(c(1, 0, 0, 1), nrow = 2, byrow = TRUE)
hypothesis_vector <- c(0, 1)

wald_statistic <- t(L %*% coef_estimates - hypothesis_vector) %*% solve(L %*% cov_matrix %*% t(L)) %*% (L %*% coef_estimates - hypothesis_vector)
...
pseudo=pseudosurv(time=test$time,event=test$status,tmax=times)$pseudo
long_data=test

long_data$pseudo=pseudo
long_data$ipseudo=1-long_data$pseudo
long_data$id=1:nrow(long_data)

long_data$risk_score=c(predicted_probs)
long_data$log_hazard=log(-log(1-long_data$risk_score))

long_data=subset(long_data, abs(ipseudo)<3)

geese_fit <- geese(as.formula(ipseudo ~ offset(log_hazard)) , id=id, data = long_data, scale.fix=TRUE, family=gaussian, mean.link = "cloglog", corstr="independence")
...
wald_statistic <- t(L %*% coef_estimates) %*% pseudoinverse(L %*% cov_matrix %*% t(L)) %*% (L %*% coef_estimates)
```
...
for (train_index in 4:4){
  for (test_index in 3:4){
    ...
    model_prune=rpart(Surv(time, status) ~ .,data=train,minbucket=20)
    ...
    pseudo=exp(-pseudo(survfit(Surv(time, status) ~ 1, data=test),times=times,type="cumhaz"))
    # pseudo=pseudosurv(time=test$time,event=test$status,tmax=times)$pseudo
    long_data=test
    ...
    wald_statistic <- t(L %*% coef_estimates) %*% pseudoinverse(L %*% cov_matrix %*% t(L)) %*% (L %*% coef_estimates)
    coxph[train_index,test_index]=wald_statistic
    time_points=summary_fit$time
    
    test_risk_score=list()
    for (i in 1:num_leaf){
      test_risk_score[[i]]=list()
      for (u in 1:length(time_points)){
    ...
    prob_matrix = matrix(unlist(test_risk_score[[1]]), ncol = length(time_points), byrow = FALSE)

    
    long_data$risk_score=c(prob_matrix)
    long_data$log_hazard=log(-log(1-long_data$risk_score))
    long_data2=subset(long_data, abs(log_hazard)<5)
    
    geese_fit <- geese(as.formula(ipseudo ~ offset(log_hazard)) , id=id, data = long_data2, scale.fix=TRUE, family=gaussian, mean.link = "cloglog", corstr="independence")
    ...
    wald_statistic <- t(L %*% coef_estimates) %*% pseudoinverse(L %*% cov_matrix %*% t(L)) %*% (L %*% coef_estimates)
    initial[train_index,test_index]=wald_statistic
    
    
    result=0
    for (i in 1:num_leaf){
      result=result+parameter[i]*matrix(unlist(test_risk_score[[i]]), ncol = length(time_points), byrow = FALSE) # calculating the ensemble risk score
    }
    
    
    long_data$risk_score=c(result+0.000001)
    long_data$log_hazard=log(-log(1-long_data$risk_score))
    long_data2=subset(long_data, abs(log_hazard)<5)
    ...
    wald_statistic <- t(L %*% coef_estimates) %*% pseudoinverse(L %*% cov_matrix %*% t(L)) %*% (L %*% coef_estimates)
    ensemble[train_index,test_index]=wald_statistic
  
    
    test_predict=predict(model1, data=test)
    chf_predict=test_predict$chf
    chf_predict_time=test_predict$unique.death.times
    test_risk_score2=matrix(0,nrow=nrow(test),ncol=length(time_points))
    ...
    long_data$risk_score=c(t(test_risk_score2)+0.000001)
    long_data$log_hazard=log(-log(1-long_data$risk_score))
    long_data2=subset(long_data, abs(log_hazard)<5)
    ...
    wald_statistic <- t(L %*% coef_estimates) %*% pseudoinverse(L %*% cov_matrix %*% t(L)) %*% (L %*% coef_estimates)
    forest[train_index,test_index]=wald_statistic
