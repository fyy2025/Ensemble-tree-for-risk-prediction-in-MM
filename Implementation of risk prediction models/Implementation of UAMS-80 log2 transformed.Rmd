...
# 66 genes from brainarray, 8 genes from bioMart
```
...
autoplot(PFS_fit,main="PFS of different risk groups categorized by UAMS-80")
```
```{r}
...
autoplot(OS_fit,main="OS of different risk groups categorized by UAMS-80")
```
...
write.csv(UAMS_score,"UAMS_80_score.csv",row.names = F)
```
