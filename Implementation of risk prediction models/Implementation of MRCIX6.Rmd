---
title: "Implementation of MRCIX6"
...
Get the probe set IDs and weights in the MRCIX6 model
```{r}
library(geneClassifiers)
MRCIX6Classifier<-getClassifier("MRCIX6")
MRCIX6Classifier
weights=getWeights(MRCIX6Classifier)
weights
...
TPM_first_MRCIX6 = subset(TPM_first_visit,Gene %in% dict$ensembl_gene_id)
tidydata=t(TPM_first_MRCIX6)%>%
...
autoplot(PFS_fit,main="PFS of different risk groups categorized by MRCIX6")
```
```{r}
...
autoplot(OS_fit,main="OS of different risk groups categorized by MRCIX6")
```
```{r}
...
```{r}
MRCIX6_score=data.frame(score)
MRCIX6_score$patient=gsub(pattern = "_1_BM_CD138pos",replacement = "",x = patient)
write.csv(MRCIX6_score,"MRCIX6_score.csv",row.names=F)
```
