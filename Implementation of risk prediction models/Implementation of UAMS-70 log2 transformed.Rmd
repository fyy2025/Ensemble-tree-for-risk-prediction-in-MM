---
title: "Implementation of UAMS-70"
...
Get the probe set IDs and weights in the UAMS-70 model
```{r}
library(geneClassifiers)
UAMS70Classifier<-getClassifier("UAMS70")
UAMS70Classifier
weights=getWeights(UAMS70Classifier)
weights
...
autoplot(PFS_fit,main="PFS of different risk groups categorized by UAMS-70")
```
```{r}
...
autoplot(OS_fit,main="OS of different risk groups categorized by UAMS-70")
```
...
UAMS_score$patient=gsub(pattern = "_1_BM_CD138pos",replacement = "",x = patient)
write.csv(UAMS_score,"UAMS_score.csv",row.names = F)
```
