...
```{r}
find_terminal_node=function(learner,observation){
  index=1
  while(!iai::is_leaf(learner,index)){
    feature=iai::get_split_feature(learner,index)
    value=iai::get_split_threshold(learner,index)
    if (observation[[feature]]<value){
      index=iai::get_lower_child(learner,index)
    }
    else{
      index=iai::get_upper_child(learner,index)
    }
  }
...
train = drop_na(MMRF)
test = drop_na(MMRF)
X=train[,1:(ncol(train)-2)]
y=train[,(ncol(train)-1):(ncol(train))]
tau=max(train$time)
...
iai::fit(grid, X, y$status==1, y$time,
         validation_criterion = "harrell_c_statistic")

model=iai::get_learner(grid)

group=rep(0,dim(train)[1])
for (i in 1:length(group)){
  group[i]=find_terminal_node(model,train[i,])
}
...
reassign_dict=as.vector(1:length(table(group)))
...
for (i in 1:length(test_group)){
  test_group[i]=find_terminal_node(model,test[i,])
}
...
risk_score=survival:::predict.coxph(baseline_model,test,type = "risk")
...
```{r}
ost_initial_loss01=rep(0,500)
ost_super_loss01=rep(0,500)
forest_loss01=rep(0,500)
num_leaves01=rep(0,500)

for (seed in 1:500){
  set.seed(seed)
  train_index = sample(nrow(merged_data), 500, replace = F)
  ...
  ost_super_loss01[seed]=cIndex(test$time,test$status,as.vector(result))[1]
  ost_initial_loss01[seed]=test_c_index[1]
  
  test_predict=ranger:::predict.ranger(model1,test)
  chf_predict=test_predict$chf
  chf_predict_time=test_predict$unique.death.times
  test_risk_score2=rep(0,nrow(test))
...
```{r}
ost_summary=cbind(forest_loss,forest_loss01,ost_initial_loss,ost_initial_loss01,ost_super_loss,ost_super_loss01,num_leaves,num_leaves01)
par(cex.axis=0.8, mar=c(5, 10, 5, 1))
boxplot(ost_summary[,1:6],beside=T,horizontal=T,las=1,xlab="C index")
# write.csv(ost_summary,"ost c index.csv",row.names = F)
```
