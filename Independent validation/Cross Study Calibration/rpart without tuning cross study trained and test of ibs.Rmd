...
test=drop_na(Gamer)
tau=max(train$time)
k=10 # number of time points used for IBS
...
fn=function(x){
  ...
  for (i in 1:num_leaf){
    result=result+x[i]*probability[[i]]
  }
  fn=-integrated_brier_score(y_true=with(train,Surv(time,status)),surv=1-result,times=seq(tau/k, tau, tau/k))
...
for (i in 1:num_leaf){
  test_ibs[i]=IBS(with(test,Surv(time,status)),sp_matrix = matrix(unlist(test_risk_score[[i]]), ncol = k, byrow = FALSE))
}
