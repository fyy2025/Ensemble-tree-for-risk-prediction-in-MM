...
pmcalibration(y=with(test,Surv(time,status)),p=test_risk_score, smooth = "rcs", nk=5, ci = "pw",time=tau)
...
tau=max(test$time)
k=10
sp_matrix=matrix(NA,nrow=nrow(test),ncol=k)
for (i in 1:k){
  for (j in 1:nrow(test)){
    t=tau/k*i
    index=length(chf_predict_time) # find the index of the largest time value smaller than t
    ...
    sp_matrix[j,i]=1-exp(-chf_predict[j,index])
  }
}
...
IBS(with(test,Surv(time,status)),sp_matrix = sp_matrix)
...
IBS(with(test,Surv(time,status)),sp_matrix = test_risk_score2,IBSrange=seq(tau/k, tau, tau/k))
```
...
#     tau=max(test$time)
#     timeOfInterest=seq(tau/10,tau,tau/10)
#     baseline_model=coxph(Surv(time, status) ~., data = train, x=TRUE, y=TRUE)
#     surv_fit <- survfit(baseline_model, newdata = test, extend=TRUE)
#     predicted_probs <- summary(surv_fit, times = timeOfInterest, extend=TRUE)$surv
#     predicted_probs_matrix <- t(predicted_probs)
#     ibs_result <- integrated_brier_score(y_true=with(test,Surv(time,status)),surv=predicted_probs_matrix,times=timeOfInterest)
# 
#     coxph[train_index,test_index]=ibs_result
#   }
# }
    structure=TRUE
    model1=ranger(Surv(time, status) ~ .,data=train,num.tree=500,mtry=ncol(train)-2,splitrule = "logrank", min.node.size = 5)
    ...
    model_prune=rpart(Surv(time, status) ~ .,data=train,minbucket=40)
      ...
      for (j in 1:nrow(CV_test)){
        ...
        for (i in 1:num_leaf){
          ...
          for (u in 1:k){
            ...
            risk_score[[i]][[u]]=c(risk_score[[i]][[u]],1-exp(-value[index]))
          }
        }
      }
    ...
    risk_score_matrix=list()
    # Remove the 0 at the beginning of each risk score
    for (i in 1:num_leaf){
      risk_score_matrix[[i]]=list()
      for (u in 1:k){
        risk_score_matrix[[i]][[u]]=risk_score[[i]][[u]][-1]
      }
    }
    ...
    probability=list()
    for (i in 1:num_leaf){
      probability[[i]]=matrix(unlist(risk_score_matrix[[i]]), ncol = k,byrow = FALSE)
    }
    # Now risk_score is a list with length=num_leaf. The ith element of risk_score is the failure probability of each patient predicted using the tree-like model with num_leaf-i+1 leaves
    ...
    fn=function(x){
      result=matrix(0,nrow=nrow(train),ncol=k)
      for (i in 1:num_leaf){
        result=result+x[i]*(1-probability[[i]])
      }
      fn=-integrated_brier_score(y_true=with(train,Surv(time,status)),surv=result,times=seq(tau/k, tau, tau/k))
    ...
    test_ibs=rep(0,num_leaf)
    for (i in 1:num_leaf){
      test_ibs[i]=integrated_brier_score(y_true=with(test,Surv(time,status)),surv=1-matrix(unlist(test_risk_score[[i]]), ncol = k, byrow = FALSE),times=seq(tau/k, tau, tau/k))
    }
    
    result=0
    for (i in 1:num_leaf){
      result=result+parameter[i]*(1-matrix(unlist(test_risk_score[[i]]), ncol = k, byrow = FALSE)) # calculating the ensemble risk score
    }
    
    # print(paste("ensemble: ",IBS(with(test,Surv(time,status)),sp_matrix =result)))
    # print(paste("initial: ",test_ibs[1]))
    ensemble[train_index,test_index]=integrated_brier_score(y_true=with(test,Surv(time,status)),surv=result,times=seq(tau/k, tau, tau/k))
    initial[train_index,test_index]=test_ibs[1]
    ...
    forest[train_index,test_index]=integrated_brier_score(y_true=with(test,Surv(time,status)),surv=1-test_risk_score2,times=seq(tau/k, tau, tau/k))


    baseline_model=coxph(Surv(time, status) ~., data = train, x=TRUE, y=TRUE)
    times = seq(tau/k, tau, by = tau/k)
    surv_fit <- survfit(baseline_model, newdata = test, extend=TRUE)
    predicted_probs <- summary(surv_fit, times = times, extend=TRUE)$surv
    predicted_probs_matrix <- t(predicted_probs)
    ibs_result <- integrated_brier_score(y_true=with(test,Surv(time,status)),surv=predicted_probs_matrix,times=seq(tau/k, tau, tau/k))
    # ibs_result <- pec(object = result,
    #                   formula = baseline_model$formula,
    #                   data = test,
    #                   times = seq(tau/k, tau, by = tau/k),
    #                   cens.model = "cox",
    #                   exact = TRUE)
    #print(paste("coxph: ",crps(ibs_result)[2]))
    coxph[train_index,test_index]=ibs_result
