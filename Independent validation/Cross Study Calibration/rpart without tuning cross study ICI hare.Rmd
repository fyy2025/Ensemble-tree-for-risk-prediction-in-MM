...
```{r}
ICI=function(test,predicted_prob,tau){
  predicted_prob=predicted_prob+0.000001
  predicted_prob=ifelse(predicted_prob<1,predicted_prob,0.99999)
...
```
```{r}
for (train_index in 4){
  for (test_index in 2){
    
    train=drop_na(datasets[[train_index]])%>%
      select(time,status,IFM15)
    ...
    predicted_probs <- c(summary(surv_fit, times = tau)$surv)
    # print(paste("coxph: ",cIndex(test$time,test$status,as.vector(risk_score))[1]))
    coxph[train_index,test_index]=ICI(test,1-predicted_probs,tau)
  }
}
A=coxph
mean(A[row(A)==col(A)])
...
for (variable in 7:7){
  for (train_index in 1:4){
    for (test_index in 1:4){
      ...
      fn=function(x){
        ...
        fn=ICI(data.frame(time=row_time,status=row_event),result,tau)
      ...
      initial_ici=ICI(test,test_risk_score[[1]],tau)
      ...
      ensemble[train_index,test_index]=ICI(test,result,tau)
      # print(paste("initial: ",test_c_index[1]))
      initial[train_index,test_index]=initial_ici
      ...
      forest[train_index,test_index]=ICI(test,test_risk_score2,tau)
  ...
  A=ensemble
  ensemble_ici[variable]=mean(A[row(A)!=col(A)])
  A=coxph
  coxph_ici[variable]=mean(A[row(A)!=col(A)])
  A=initial
  initial_ici[variable]=mean(A[row(A)!=col(A)])
  A=forest
  forest_ici[variable]=mean(A[row(A)!=col(A)])
}
```
```{r}
