...
k=10
baseline_model=coxph(Surv(time, status) ~., data = train, x=TRUE, y=TRUE)
times = seq(tau/k, tau/k*(k-4), tau/k)
surv_fit <- survfit(baseline_model, newdata = test)
summary_fit=summary(surv_fit, times = times, extend=TRUE)
predicted_probs <- summary_fit$surv
...
pseudo=pseudosurv(time=test$time,event=test$status,tmax=summary_fit$time)$pseudo

b <- NULL
for(j in 1:ncol(pseudo)){
  b <- rbind(b,cbind(test,pseudo=pseudo[,j], tpseudo=times[j],id=1:nrow(test)))
}
b <- b[order(b$id),]
library(geepack)
b$tpseudo <- as.factor(b$tpseudo)
...
long_data$risk_score=c(t(predicted_probs))
long_data$log_hazard=log(-log(1-long_data$risk_score))

long_data=subset(long_data, abs(ipseudo)<3)

geese_fit <- geese(as.formula(ipseudo ~ tpseudo+offset(log_hazard)) , id=id, data = long_data, scale.fix=TRUE, family=gaussian, mean.link = "cloglog", corstr="independence")
...
for (train_index in 1:4){
  for (test_index in 1:4){
    ...
    long_data$risk_score=c(t(predicted_probs))
    long_data$log_hazard=log(-log(1-long_data$risk_score))
    long_data=subset(long_data, abs(log_hazard)<5)
    long_data=subset(long_data, abs(ipseudo)<3)
