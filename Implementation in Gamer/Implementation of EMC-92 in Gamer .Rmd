...
Create list of all the symbol ids in the EMC model and record thier revised weights
```{r}
first=0
second=0
revised_weight=c()
...
KM=autoplot(PFS_fit,main="PFS of different risk groups in Gamer categorized by EMC-92")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/Gamer-EMC92-PFS.pdf",
       device = "pdf")
```
```{r}
...
KM=autoplot(OS_fit,main="OS of different risk groups in Gamer categorized by EMC-92")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/Gamer-EMC92.pdf",
       device = "pdf")
write(high_risk, "./Categorization/Gamer-EMC92.txt")
```
```{r}
...
write.csv(EMC_score,"EMC92_score_Gamer.csv",row.names=F)
```
