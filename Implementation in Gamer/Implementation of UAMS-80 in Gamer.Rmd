...
KM=autoplot(OS_fit,main="OS of different risk groups in Gamer categorized by UAMS-80")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/Gamer-UAMS80.pdf",
       device = "pdf")
write(high_risk, "./Categorization/Gamer-UAMS80.txt")
```
```{r}
...
write.csv(UAMS_score,"UAMS80_score_Gamer.csv",row.names=F)
```
