...
Create list of all the ensembl ids in the HM19 model and record thier revised weights
```{r}
first=0
second=0
revised_weight=c()
...
KM=autoplot(OS_fit,main="OS of different risk groups in Gamer categorized by HM-19")+
  ggplot2::annotate("text", x = 10, y = 0.01, label = paste0("pvalue: ", sprintf("%.2e", result$pvalue)) , size = 3, hjust = 0)
ggsave(KM, 
       filename = "./KM\ curves/Gamer-HM19.pdf",
       device = "pdf")
write(high_risk, "./Categorization/Gamer-HM19.txt")
```
```{r}
...
```{r}
HM19_score=data.frame(score)
HM19_score$patient=patient
write.csv(HM19_score,"HM19_score_Gamer.csv",row.names=F)
```
