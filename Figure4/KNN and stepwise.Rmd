...
```{r}
stepwise_cox <- function(formula, data, direction = "both", tau) {
...
final_model <- stepwise_cox(as.formula(Surv(time, status) ~ EMC+UAMS70+GPI+UAMS17+UAMS80+IFM15+HM19), data=MMRF, tau=365*8)
dd <- datadist(MMRF)
options(datadist="dd")
summary(final_model)
```
