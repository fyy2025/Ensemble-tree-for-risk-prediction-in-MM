...
stepwise_cox <- function(formula, data, direction = "both", datasets = list(MMRF, IFM, UAMS, Gamer), tau) {
  custom_loss <- function(model){
    ...
    for (i in 1:length(datasets)){
      total_event = total_event + sum(datasets[[i]]$status)
    }
    ...
    for (i in 1:length(datasets)){
      ...
      weighted_ICI = weighted_ICI + ICI(dataset,1 - surv_probs,tau)*sum(dataset$status)/total_event
    }
    return (weighted_ICI)
  }
...
for (tau in seq(1,5,0.5)){
  final_model = stepwise_cox(as.formula(Surv(time, status) ~ EMC+UAMS70+GPI+UAMS17+UAMS80+IFM15+HM19), data=MMRF, tau=tau*365)
...
write.csv(MMRF_coef,"MMRF_coefficients.csv")
```
...
for (tau in seq(1,5,0.5)){
  final_model = stepwise_cox(as.formula(Surv(time, status) ~ EMC+UAMS70+GPI+UAMS17+UAMS80+IFM15+HM19), data=IFM, tau=tau*365)
...
write.csv(IFM_coef,"IFM_coefficients.csv")
```
...
for (tau in seq(1,5,0.5)){
  final_model = stepwise_cox(as.formula(Surv(time, status) ~ EMC+UAMS70+GPI+UAMS17+UAMS80+IFM15+HM19), data=UAMS, tau=tau*365)
...
write.csv(UAMS_coef,"UAMS_coefficients.csv")
```
...
for (tau in seq(1,5,0.5)){
  final_model = stepwise_cox(as.formula(Surv(time, status) ~ EMC+UAMS70+GPI+UAMS17+UAMS80+IFM15+HM19), data=Gamer, tau=tau*365)
...
write.csv(Gamer_coef,"Gamer_coefficients.csv")
```
